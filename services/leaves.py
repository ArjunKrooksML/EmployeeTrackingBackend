...
def get_all_leaves(db: Session) -> List[dict]:
    results = db.query(LeaveDB, EmployeeDB.employee_name).join(EmployeeDB, LeaveDB.employee_id == EmployeeDB.employee_id).order_by(LeaveDB.created_at.desc()).all()
    # Format the results into a dict that Pydantic can parse
    formatted = []
